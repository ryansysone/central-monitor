...
const option = computed<EChartsOption>(() => {
  ...
  const values = sortedHistory.value.map((item) =>
    normalizePercent(item.cpuUsage)
  );
  ...
  return {
    ...
    tooltip: {
      ...
      formatter: (params) => {
        ...
        const historyItem = sortedHistory.value[dataIndex];
        ...
        return [
          `<strong>${formatChartTime(historyItem.collectedAt)}</strong>`,
          `CPU 使用率：${numericValue.toFixed(1)}%`,
        ].join("<br/>");
      },
    },
